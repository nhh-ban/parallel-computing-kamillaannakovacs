...
apply_MTweedieTests <- function(row) {
  MTweedieTests(N = row$N, M = row$M, sig = 0.05)
}
...
clusterExport(cl, varlist = c("apply_MTweedieTests", "MTweedieTests", "simTweedieTest"))

# Use parLapply to apply the function to each row of df
results <- parLapply(cl, split(df, rownames(df)), apply_MTweedieTests)
